function dx = Injury_Model_Final_Control(t,x,par)
...
%%% ODE equations
dx(1) = N*par.alpha1*act(Pathogen,par.n1,par.K1)*Capacity - par.gamma1*N*S2 - par.mu1*N ; %%%N
...
dx(5) = par.alpha3*M1*Pathogen*regulation(M1,par.threshold) - par.gamma5*N*Pathogen - par.delta8*Pathogen; %%% Pathogen 

dx(6) = control(S2,par.beta1,par.S2Ref,par.Kp)*N - par.delta1*S2; % %%% S2
